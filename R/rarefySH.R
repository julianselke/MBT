...
#' @import ggplot2 dplyr vegan shiny phyloseq

rarefySH <- function(obj){
  # taxa rows?
  ft <- otu_table(obj) %>% t() %>% data.frame() %>% `class<-`(.,"data.frame")
  md <- sample_data(obj) %>% data.frame() %>% `class<-`(.,"data.frame")
  raremax <- max(rowSums(ft))
  message("Computing Rarefaction Curve...")
  rmq <<- vegan::rarecurve(ft, step = 20, sample = raremax, tidy = T) %>%
    group_by(Site) %>% mutate(Max = max(Sample)) %>% data.frame()
  m <<- rmq[,c(1,4)] %>% unique.data.frame() %>%
    `colnames<-`(c(colnames(md)[1],"max")) %>% merge(., md, colnames(md)[1])
  pal <- c('#636EFA', '#EF553B', '#00CC96', '#AB63FA', '#FFA15A', '#19D3F3',
           '#FF6692', '#B6E880', '#FF97FF', '#FECB52')
  if (interactive()) {
    ui <- fluidPage(
      fluidRow(plotOutput("depthPlot")),
      fluidRow(column(12,align="center",
                      sliderInput("depth", "Rarefaction Depth:", min = 0,
                                  max = max(rmq$Sample), value = raremax,
                                  width = "50%"))),
      fluidRow(column(8,align="right",
                      radioButtons(inputId="group", inline = TRUE,
    ...
    server <- function(input, output) {
      output$depthPlot <- renderPlot({
        ggplot() + geom_line(data = rmq,
                             aes(x = Sample, y = Species, group = Site),
                             color = "gray") + theme_bw() +
          theme(legend.position = "none") +
          geom_line(data=rmq[rmq$Max >= input$depth,],
                    aes(x=Sample,y=Species, group=Site), color = '#636EFA') +
          geom_vline(xintercept = input$depth)
      })
      output$rarePlot <- renderPlot({
        p <- ggplot(m) +
          geom_col(aes(x = m[[input$group]], y = max), fill = "gray") +
          geom_col(data = m[m$max >= input$depth , ],
                   aes(x = m[m$max >= input$depth , ][[input$group]],
                       y = max,
                       fill = m[m$max >= input$depth , ][[input$group]])) +
          geom_hline(yintercept = input$depth) + theme_bw() +
          theme(legend.position = "none",
