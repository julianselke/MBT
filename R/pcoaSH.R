...
pcoaSH <- function(obj) {
  ...
  ui <- fluidPage(
    tags$head(
      tags$style(HTML("
       ...
       ")),
      # https://stackoverflow.com/questions/17325521/r-shiny-display-loading-\
      # message-while-function-is-running
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        pickerInput(
          ...
          label = "part.weights",
          choices = c(
            "uniform", "gm.counts", "anorm", "anorm.x.gm.counts",
  ...
  server <- function(input, output) {
    ...
    idata <- reactive({
      MBT:::dist(
        obj = subObj(),
        dis = distList$id[distList$name == input$dist],
        pseudocount = 1e-12,
        ilr.weights = input$ilr.weights,
    ...
    output$pcoa <- renderPlotly({
      pcoa <- ape::pcoa(idata())
      data <- data.frame(
        Sample = as.character(rownames(pcoa$vectors)),
        X = pcoa$vectors[, 1],
        Y = pcoa$vectors[, 2],
        Z = pcoa$vectors[, 3]
      )
      ...
      axz <- list(
        title = paste("PCo 3 - ",
          round(pcoa$values$Relative_eig[[3]] * 100, 2),
          "%",
          sep = ""
        ),
        tickfont = list(color = c("#ababab")), color = "#777777"
