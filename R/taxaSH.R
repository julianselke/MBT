  ...
  ui <- fillPage(
    tags$head(
      tags$style(HTML("
       ...
       ")),
      # https://stackoverflow.com/questions/17325521/r-shiny-display\
      #-loading-message-while-function-is-running
...
taxa.plot <- function(obj, tax, variable, facet_var = NULL, add_meta = TRUE) {
  ...
  d <- data[, c(
    1:(ncol(data) - ncol(sample_data(obj))),
  ...
  if (is.null(facet_var)) {
    ...
    z <- z %>%
      ...
      mutate(rel_ab = sum(value / total * 100))
    p <- z %>%
      ggplot(., aes(
        x = data, y = value / total, fill = variable, col = variable,
        text = paste(
          data, "\n",
          "Rank:                    ", variable, "\n",
  ...
  if (!is.null(facet_var)) {
    ...
    z <- z %>%
      group_by(facet, data) %>%
      mutate(facet_total = sum(value))
    z <- z %>%
      group_by(facet) %>%
      mutate(plot_width = length(unique(data)))
    plot_widths <- z %>%
      group_by(facet) %>%
      summarise(plot_width)
    ...
    p <- z %>%
      ...
      subplot(margin = 0.005, shareY = TRUE, widths = plot_widths)
  }
  return(p)
}
